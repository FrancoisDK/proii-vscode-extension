...
// Store stream descriptions for hover provider
...
export const streamDescriptions = new Map<string, string>();
...
export class StreamNameProvider implements vscode.DocumentSemanticTokensProvider {
     ...
     */
    private isProIIKeyword(word: string): boolean {
     ...
     */
    private isInStreamContext(line: string, word: string, position: number): boolean {
        // Skip if in comment
     ...
     */
    private findPotentialStreamReferences(document: vscode.TextDocument): Map<string, { line: number, start: number, length: number }[]> {
        const potentialStreams = new Map<string, { line: number, start: number, length: number }[]>();
        ...
        const resultRanges: { start: number, end: number }[] = [];
        for (let i = 0; i < lineCount; i++) {
        ...
        const isInResultRange = (ln: number) => {
            ...
            if (nameLineMatch) {
                ...
                if (streamName !== 'NAME' && !this.isProIIKeyword(streamName) && this.shouldHighlightStreamName(streamName)) {
                    ...
                    if (descriptionMatch) {
                        ...
                        if (descriptions) {
                            streamDescriptions.set(streamName, descriptions);
...
export const semanticTokenLegend = new vscode.SemanticTokensLegend(
    ['streamName'], // types: 0 = defined streams (blue only)
    [''] // modifiers
);
