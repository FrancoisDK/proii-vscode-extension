...
exports.ColumnLimiterProvider = void 0;
const vscode = __importStar(require("vscode"));
class ColumnLimiterProvider {
    constructor() {
        this.columnLimit = 80;
    ...
    checkDocument(document) {
        ...
        this.enableDiagnostics = config.get('columnLimiter.enabled', true);
        this.columnLimit = config.get('columnLimiter.columnLimit', 80);
        const warnOnExceed = config.get('columnLimiter.warnOnExceed', true);
        ...
        for (let i = 0; i < document.lineCount; i++) {
            ...
            if (lineText.length > this.columnLimit) {
                ...
                const range = new vscode.Range(new vscode.Position(i, this.columnLimit), new vscode.Position(i, lineText.length));
                const diagnostic = new vscode.Diagnostic(range, `Line exceeds ${this.columnLimit} character limit by ${exceededBy} character${exceededBy > 1 ? 's' : ''} (Pro/II typically truncates at column 80)`, vscode.DiagnosticSeverity.Warning);
    ...
    provideCodeActions(document, range, context, _token) {
        ...
        const relevantDiagnostics = context.diagnostics.filter((diag) => diag.code === 'proii-column-limit');
        if (relevantDiagnostics.length === 0) {
            return actions;
        }
        const config = vscode.workspace.getConfiguration('proii');
        const enableAutoFix = config.get('columnLimiter.enableAutoFix', true);
        if (!enableAutoFix) {
            return actions;
        }
        // Action 1: Truncate line to column limit
        const truncateAction = new vscode.CodeAction(`Truncate line to ${this.columnLimit} characters`, vscode.CodeActionKind.QuickFix);
        ...
        if (line.text.length > this.columnLimit) {
            ...
            truncateAction.edit.replace(document.uri, line.range, truncatedText);
        }
        actions.push(truncateAction);
        // Action 2: Continue line (add & continuation marker and break to next line)
        const continueAction = new vscode.CodeAction('Continue line with & marker (comment)', vscode.CodeActionKind.QuickFix);
        ...
        if (line.text.length > this.columnLimit) {
            ...
            continueAction.edit.replace(document.uri, line.range, firstPart + '\n' + secondPart);
        }
        actions.push(continueAction);
        // Action 3: Add comment line with truncated content
        const truncateWithCommentAction = new vscode.CodeAction(`Add comment with truncated content`, vscode.CodeActionKind.QuickFix);
        truncateWithCommentAction.edit = new vscode.WorkspaceEdit();
        if (line.text.length > this.columnLimit) {
            const truncatedText = line.text.substring(0, this.columnLimit);
            const truncatedPart = line.text.substring(this.columnLimit);
            truncateWithCommentAction.edit.replace(document.uri, line.range, truncatedText + '\n$ ' + truncatedPart);
        }
        actions.push(truncateWithCommentAction);
        // Action 4: Ignore this line (disable warning)
        const ignoreAction = new vscode.CodeAction('Disable this line (add to ignore)', vscode.CodeActionKind.QuickFix);
...
exports.ColumnLimiterProvider = ColumnLimiterProvider;
//# sourceMappingURL=columnLimiterProvider.js.map